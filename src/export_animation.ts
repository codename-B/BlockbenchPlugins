 ...
 * When `nonLinear` is supplied, the animation's name is appended to it if any of its keyframes
 * use a non-linear interpolation, so the caller can show one aggregated warning instead of a
 * popup per animation.
 */
export function compile_animation(animation: _Animation, nonLinear?: string[]): VS_Animation | null {
    ...
    if (hasNonLinearInterpolation && nonLinear) nonLinear.push(animation.name);
    return vsAnimation;
}
